...
namespace BackToThePast.OldXO
{
    ...
    public static class StartPatch
    {
        public static void Postfix()
        {
            ...
            for (int i = 0; i < 15; i++)
                if (i == 2)
                    FloorUtils.AddEventFloor(5 + i, 23, () => scrCamera.instance.positionState = PositionState.CrownIsland, true, oldXO.transform).gameObject.AddComponent<FloorHider>();
                else if (i == 3)
                    FloorUtils.AddEventFloor(5 + i, 23, () => scrCamera.instance.positionState = (PositionState)1001, true, oldXO.transform).gameObject.AddComponent<FloorHider>();
                else if (i == 10)
                    FloorUtils.AddEventFloor(5 + i, 23, () => {
                        if (PlayerPrefs.GetInt("BackToThePast.FoundSecretIsland", 0) == 0)
                        {
                            PlayerPrefs.SetInt("BackToThePast.FoundSecretIsland", 1);
                            PlayerPrefs.Save();
                        }
                    }, false, oldXO.transform);
                else if (i > 10)
                    FloorUtils.AddFloor(5 + i, 23, oldXO.transform);
                else
                    FloorUtils.AddEventFloor(5 + i, 23, null, true, oldXO.transform).gameObject.AddComponent<FloorHider>();
            scrFloor normal = FloorUtils.AddEventFloor(18, 30, () => OldXOTweak.EnterLevel(false, false), true, oldXO.transform);
            ...
            scrFloor speed = FloorUtils.AddEventFloor(20, 30, () => OldXOTweak.EnterLevel(true, false), true, oldXO.transform);
            speed.isportal = true;
            ...
            scrFloor portal = FloorUtils.AddEventFloor(19, 30, () =>
            {
            ...
            }, true, oldXO.transform);
            FloorUtils.AddEventFloor(19, 24, () => {
            ...
            FloorUtils.AddEventFloor(19, 25, () => {
